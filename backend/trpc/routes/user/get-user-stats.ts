...
export const getUserStatsProcedure = protectedProcedure.query(async ({ ctx }) => {
  ...
  const results = await Promise.allSettled([
    ...
    supabase.from('colorings').select('*', { count: 'exact', head: true }).eq('user_id_fk', userId),
    supabase.from('users').select('quota_used, created_at').eq('id', userId).single(),
  ]);
  ...
  const stats = {
    ...
    totalAnalyses: (userResult.data?.quota_used as Record<string, number> | null)?.analyses || 0,
