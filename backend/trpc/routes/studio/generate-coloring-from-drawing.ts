...
const THERAPEUTIC_COLORING_THEMES: Record<string, { theme: string; elements: string[] }> = {
  war: {
    theme: 'peaceful garden with protective walls',
    elements: ['flowers', 'rainbow', 'protective trees', 'happy birds'],
  },
  violence: {
    theme: 'calm forest with strong friendly animals',
    elements: ['gentle giant', 'protective bear', 'safe cave'],
  },
  disaster: {
    theme: 'rebuilding together scene',
    elements: ['helping hands', 'new house', 'rainbow after rain', 'community'],
  },
  loss: {
    theme: 'memory garden with stars',
    elements: ['heart-shaped cloud', 'eternal flower', 'guiding star'],
  },
  loneliness: {
    theme: 'making friends scene',
    elements: ['welcoming animals', 'open door', 'friendship bridge'],
  },
  fear: {
    theme: 'brave little hero',
    elements: ['flashlight', 'protective shield', 'friendly monster becoming small'],
  },
  abuse: {
    theme: 'safe castle with protectors',
    elements: ['guardian angel', 'strong walls', 'loving family'],
  },
  family_separation: {
    theme: 'two homes connected by love',
    elements: ['heart bridge', 'two houses', 'connected by rainbow'],
  },
  death: {
    theme: 'butterfly transformation',
    elements: ['caterpillar to butterfly', 'stars', 'memory tree'],
  },
  neglect: { theme: 'warm caring home', elements: ['cozy bed', 'full table', 'loving hands'] },
  bullying: {
    theme: 'friendship circle',
    elements: ['diverse friends holding hands', 'strength together', 'kind words'],
  },
  domestic_violence_witness: {
    theme: 'peaceful safe haven',
    elements: ['quiet garden', 'protective bubble', 'harmony'],
  },
  parental_addiction: {
    theme: 'sunshine breaking through clouds',
    elements: ['hope flower', 'helping hands', 'bright future'],
  },
  parental_mental_illness: {
    theme: 'love always present',
    elements: ['heart sun', 'patient flower', 'strong little one'],
  },
  medical_trauma: {
    theme: 'brave superhero healing',
    elements: ['healing powers', 'friendly doctor', 'strength cape'],
  },
  anxiety: {
    theme: 'calm breathing exercise',
    elements: ['peaceful lake', 'gentle breeze', 'relaxing cloud'],
  },
  depression: {
    theme: 'colors returning to world',
    elements: ['sun peeking', 'rainbow appearing', 'flower blooming'],
  },
  low_self_esteem: {
    theme: 'unique special star',
    elements: ['crown', 'mirror of beauty', 'shining from inside'],
  },
  anger: {
    theme: 'taming the anger dragon',
    elements: ['calm dragon', 'breathing exercise', 'emotion master'],
  },
  school_stress: {
    theme: 'learning adventure',
    elements: ['happy school', 'fun books', 'celebrating mistakes'],
  },
  social_rejection: {
    theme: 'finding true friends',
    elements: ['unique character celebrated', 'inclusive circle', 'belonging'],
  },
  displacement: {
    theme: 'home in the heart',
    elements: ['portable home', 'family together', 'new adventures'],
  },
  poverty: {
    theme: 'richness of love',
    elements: ['heart treasures', 'sharing joy', 'family bond'],
  },
  cyberbullying: {
    theme: 'digital safety hero',
    elements: ['shield from screen', 'real friends', 'safe online'],
  },
};
 ...
 */
async function _toLineArt(input: Buffer): Promise<Buffer> {
...
Respond in JSON format:
{
  ...
  "contentAnalysis": {
    ...
    "therapeuticApproach": "suggested approach if any"
      ...
      const contentAnalysis = parsedAnalysis.contentAnalysis || {
      ...
      if (contentAnalysis.hasConcerningContent) {
        logger.info(
          '[Generate Coloring] ⚠️ CONCERNING CONTENT DETECTED:',
      ...
      if (contentAnalysis.hasConcerningContent && contentAnalysis.concernType) {
        ...
        if (therapeuticTheme) {
          ...
          logger.info(
            '[Generate Coloring] 💜 Therapeutic coloring theme:',
            therapeuticColoringTheme
          );
      ...
      logger.info(
        '[Generate Coloring] 📝 Professional prompt generated:',
        flux2Prompt.substring(0, 150) + '...'
      );
      logger.info(
        '[Generate Coloring] 🚫 Negative prompt:',
        negativePrompt.substring(0, 100) + '...'
      );
