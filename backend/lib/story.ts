...
import { generateTextOverlayHTML, generateFontImports } from './text-overlay.js';
...
export async function generateImageForPage(
  ...
  const finalPrompt =
    prompt ||
    `Children's storybook illustration, soft pastel watercolor, simple rounded shapes, warm friendly atmosphere, plain background, NO TEXT NO LETTERS, scene: ${text}`;
...
export async function makeStorybook(opts: MakeOptions) {
  ...
  const seed = generateStorybookSeed(opts.user_id || 'anonymous', Date.now());
  ...
  const imagePromises = opts.pages.map(async (page, i) => {
    try {
      ...
      const url = await uploadBuffer(
        BUCKET,
        `images/story_${Date.now()}_${i + 1}.png`,
        png,
        'image/png'
      );
  ...
  if (opts.makePdf) {
    ...
    try {
      ...
      pdf_url = await uploadBuffer(
        BUCKET,
        `pdf/story_${Date.now()}.pdf`,
        Buffer.from(pdf),
        'application/pdf'
      );
  ...
  if (opts.makeTts) {
    try {
      logger.info('[Story] Generating TTS audio in parallel');
      const ttsPromises = opts.pages.map(async (page, i) => {
        ...
        return uploadBuffer(BUCKET, `audio/story_${Date.now()}_${i + 1}.mp3`, mp3, 'audio/mpeg');
      });
      voice_urls = await Promise.all(ttsPromises);
