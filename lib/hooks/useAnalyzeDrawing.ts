...
export interface UseAnalyzeDrawingReturn {
  ...
  error: string | null;
  isLoading: boolean;
  analyze: (payload: { taskType: string; childAge?: number; imageUri?: string; imageBase64?: string; language?: "tr" | "en" }) => Promise<void>;
}
...
export function useAnalyzeDrawing(): UseAnalyzeDrawingReturn {
  ...
  const mutation = trpc.studio.analyzeDrawing.useMutation({
    ...
    onError: (err: any) => {
      ...
      const errorMessage = err?.message || "Analiz başarısız oldu";
        ...
        try {
          if (Platform.OS === 'web') {
            ...
            if (payload.imageUri.startsWith('data:')) {
              ...
              console.log("[Hook] ✅ Image extracted (data URL), size:", imageBase64.length, "bytes");
            } else {
          ...
          setError(`Resim okunamadı: ${readErr instanceof Error ? readErr.message : 'Bilinmeyen hata'}`);
          return;
        }
      }
